            ...
            document.querySelectorAll('select[id^="lora_name_"]').forEach(sel => {
                ...
                sel.addEventListener('change', async () => {
                    try {
                        ...
                        const triggers = (item.triggers || []).slice(0, 12);
                        ...
                        // 将触发词插入到快捷提示词组前置区
                        if (triggers.length) {
                            this.prependLoraPromptShortcuts(triggers, val);
                        }
                    } catch (e) {}
                });
                // 初次渲染时也触发一次，若已有默认值
    ...
    generatePromptShortcuts(analysis) {
        ...
        flatSubs.forEach(sub => {
            ...
            (sub.items || []).forEach(item => {
                ...
                btn.onclick = () => { this.lastPresetLabel = item.label; this.applyPromptShortcut(item); };
    ...
    /* 原始旧数据注释掉
            ...
            },
            {
                title: '生活方式 Lifestyle', badges: ['lifestyle'], items: [
                    { label: '健身运动', prompt: `${baseQuality}, fitness action, dynamic muscles, studio rim light` },
                    { label: '家居软装', prompt: `${baseQuality}, home decor, tasteful interior styling, natural daylight` },
                    { label: '旅行记录', prompt: `${baseQuality}, travel destination, storytelling frame, local culture` }
                ]
            },
            {
                title: '健康 Health', badges: ['health'], items: [
                    { label: '心理健康', prompt: `${baseQuality}, mental well-being concept, serene visuals, mindful composition` },
                    { label: '健康饮食', prompt: `${baseQuality}, healthy eating, fresh ingredients, clean styling` },
                    { label: '医疗护理', prompt: `${baseQuality}, healthcare scene, professional equipment, sterile lighting` }
                ]
            }
        ];
